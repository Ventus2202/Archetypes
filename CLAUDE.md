...
Because those cascades were inert for a long time, archives written before they were switched on can contain rows whose parent is gone. `BackupData.withoutOrphans()` drops them on the way in — the same call `_purgeOrphans` makes on the DB side — and `importFromBytes` returns a `BackupImportReport` listing what it left out, so the UI can say so. Dropping is only right for orphans scattered among sound rows: `BackupData.checkParentSectionsPresent()` runs first and refuses an archive that lost a whole parent section, because pruning that one under `replace: true` would empty the DB and report success. If you add a child table, it needs a check in **all three** places.
...
### Educational content
`assets/content/{it,en}/` holds the localized JSON content, loaded and cached per locale by `ContentRepository`: `mbti.json` (16 types, 8 cognitive functions, 4 dichotomies), `relationship_dynamics.json`, `career_roles.json`, `team_objectives.json`. Domain engines emit translation **keys** (e.g. `FrictionPoint.contentKey`); the content JSON / ARB resolve them to localized strings. A key must index the JSON entry **exactly** — the engine emits the naked key and the entry holds `title` + `description`; screens fall back to `?? key`, so a mismatch reads as a raw key on screen instead of failing. `test/assets/content_assets_test.dart` pins both sides of that contract. Access MBTI content via `getTypeContent(content, "INTJ")`, `getFunctionContent(content, "Ni")`, `getDichotomyContent(content, "IE")`. `ContentViewerScreen` takes a `contentKey` + `ContentViewerType` enum and dispatches to the correct section.
